...
public class PoisonEnchantment extends Enchantment {
    public PoisonEnchantment(Rarity weight, EnchantmentTarget target, EquipmentSlot... slotTypes) {
    ...
    public void onTargetDamaged(LivingEntity user, Entity target, int level) {
        if (target instanceof LivingEntity && !user.getWorld().isClient) {
            if (level == 1) {
                ...
                ((LivingEntity) target).addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 200, 5));
                ...
                for (LivingEntity entity : entities) {
                    if (!(entity instanceof PlayerEntity))
                        entity.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 200, 5));
                }
            } else if (level == 3) {
                ((LivingEntity) target).addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 200, 10));
                ...
                for (LivingEntity entity : entities) {
                    if (!(entity instanceof PlayerEntity))
                        entity.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 200, 10));
                }
            }
        }
